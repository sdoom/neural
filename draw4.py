...
#YX.handle()
#for x in YX.the_dict['root']['item']:
conf = xmlInit('config.xml',[])
conf.handle()
print(conf.the_dict['root']['学习率'].decode(encoding='UTF-8'))


# number of input, hidden and output nodes
#input_nodes = 3
input_nodes = int(conf.the_dict['root']['输入层节点'].decode(encoding='UTF-8'))

#hidden_nodes = 3
#hidden_nodes = 300
hidden_nodes = int(conf.the_dict['root']['隐藏层节点'].decode(encoding='UTF-8'))

#output_nodes = 7
output_nodes = int(conf.the_dict['root']['输出层节点'].decode(encoding='UTF-8'))
# learning rate is 0.3
learning_rate = float(conf.the_dict['root']['学习率'].decode(encoding='UTF-8'))
#learning_rate = 0.3
...
class dataPre:
    ...
    def targetOpera(self):
        for index in self.usefulIndex:
            ...
            yx = all_values[4].replace('\n','')
            self.targets.append(yx)
    ...
    def targetNormalization(self):
        YX = xmlInit('YXDM.xml',[])
        YX.handle()
        for target in range(0,len(self.targets)):
            for YxCode in YX.the_dict['root']['item']:
                if self.targets[target] == YxCode['#item']['岩性代码'] :
                    self.targets[target] = int(YxCode['#item']['编号'])
                    
        
            #print(YxCode['#item']['岩性代码'])
...
test_data_file = open("well99.csv", 'r')
