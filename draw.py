...
class dataPre:
    ...
    def slope(self,training_data_list):
        ...
        for now in range(0,slpoeSize):
            ...
            if float(next[3]) - float(tmps[3]) != 0 and float(next[0]) - float(tmps[0]) != 0 and float(next[2]) - float(tmps[2]) != 0:
            ...
            if float(next[2]) - float(tmps[2]) != 0 and float(next[0]) - float(tmps[0]) != 0 and float(next[3]) - float(tmps[3]) != 0:    
...
training_data_file = open("wellallTrain.csv", 'r')
...
test_data_file = open("well436.csv", 'r')
...
for record in test_data_list:
    ...
    # scale and shift the inputs
    inputs = (numpy.asfarray(all_values[1]) / 2000.0 * 0.99) + 0.01
    # query the network
